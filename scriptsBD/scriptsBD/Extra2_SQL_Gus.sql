...
select codigo_cliente, date_format(fecha_pago, '%Y %M') as año 
from pago where fecha = 2008 group by codigo_cliente ;
select * from pago;

-- 9. Devuelve un listado con el código de pedido, código de cliente, fecha esperada y fecha de
-- entrega de los pedidos que no han sido entregados a tiempo.

select codigo_pedido, codigo_cliente, fecha_esperada, fecha_entrega, comentarios from pedido
where fecha_entrega > fecha_esperada || comentarios like '%tarde%';
...
select codigo_pedido, codigo_cliente, fecha_esperada, fecha_entrega, datediff(fecha_esperada, fecha_entrega) as diferencia from pedido 
where datediff(fecha_esperada, fecha_entrega) >= 2;

select codigo_pedido, codigo_cliente, fecha_esperada, fecha_entrega, 
adddate(fecha_entrega, interval 2 day) as diferencia from pedido 
where adddate(fecha_entrega, interval 2 day) <= fecha_esperada;

-- 11. Devuelve un listado de todos los pedidos que fueron rechazados en 2009.
select * from pedido where estado = "rechazado" and year(fecha_pedido) = "2009" && year(fecha_esperada) = "2009";
...
select * from pago where year(fecha_pago) = 2008 && forma_pago ="PayPal" order by total desc; -- fecha_pago desc;
...
select * from producto where gama = "ornamentales" && cantidad_en_stock > 100 order by precio_venta desc;
select * from producto;

/* 16. Devuelve un listado con todos los clientes que sean de la ciudad de Madrid y cuyo
representante de ventas tenga el código de empleado 11 o 30.*/

select nombre_cliente, codigo_empleado_rep_ventas, ciudad from cliente where (codigo_empleado_rep_ventas = 11 || codigo_empleado_rep_ventas = 30) && ciudad = "madrid";
/*Consultas multitabla (Composición interna)
Las consultas se deben resolver con INNER JOIN.
1. Obtén un listado con el nombre de cada cliente y el nombre y apellido de su representante
de ventas.*/
select nombre_cliente, empleado.nombre, empleado.apellido1 from cliente 
inner join empleado on ecodigo_empleado_rep_ventasmpleado.codigo_empleado = cliente.codigo_empleado_rep_ventas; 

-- 2. Muestra el nombre de los clientes que hayan realizado pagos junto con el nombre de sus
-- representantes de ventas.

select nombre_cliente, empleado.nombre, empleado.apellido1 from cliente
 join empleado on codigo_cliente =  codigo_empleado_rep_ventas join pago on cliente.codigo_cliente = id_transaccion;
